...
const Cubes: React.FC<CubesProps> = ({
  ...
  const onPointerMove = useCallback(
    (e: PointerEvent) => {
      ...
      const cellH = rect.height / rowsCount;
  ...
  const onClick = useCallback(
    ...
    [rippleOnClick, cols, rowsCount, faceColor, rippleColor, rippleSpeed]
  );
  ...
  }, [autoAnimate, cols, rowsCount, tiltAt]);
  ...
  const rowCells = Array.from({ length: rowsCount });
  const sceneStyle: React.CSSProperties = {
    gridTemplateColumns: cubeSize ? `repeat(${cols}, ${cubeSize}px)` : `repeat(${cols}, 1fr)`,
    gridTemplateRows: cubeSize ? `repeat(${rowsCount}, ${cubeSize}px)` : `repeat(${rowsCount}, 1fr)`,
    columnGap: colGap,
    rowGap: rowGap,
  ...
  const wrapperStyle = {
    ...
    ...(!cubeSize
      ? {
          aspectRatio: `${cols} / ${rowsCount}`
        }
      : {}),
    ...(cubeSize
      ? {
          width: `${cols * cubeSize}px`,
          height: `${rowsCount * cubeSize}px`
        }
      : {})
