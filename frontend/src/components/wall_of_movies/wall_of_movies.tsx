...
export default function DomeGallery({
  ...
  const filteredImages = useMemo(
    () =>
      images.filter(img => {
        ...
        if (invalidSrcs.has(src)) return false
        return true
      }),
    [images, invalidSrcs]
  )
  ...
  const items = useMemo(
    () => buildItems(filteredImages, segments),
    [filteredImages, segments]
  )
  ...
  const startInertia = useCallback(
    (vx: number, vy: number) => {
      const MAX_V = 4
      let vX = clamp(vx, -MAX_V, MAX_V)
      let vY = clamp(vy, -MAX_V, MAX_V)
      let frames = 0
      const d = clamp(dragDampening ?? 0.6, 0, 0.1)
      const frictionMul = 0.975 + 0.015 * d
      const stopThreshold = 0.002
      const maxFrames = Math.round(360 + 360 * d)
      const step = () => {
        vX *= frictionMul
        vY *= frictionMul
        ...
        const nextX = clamp(
          rotationRef.current.x - vY * 0.5,
          -maxVerticalRotationDeg,
          maxVerticalRotationDeg
        )
        const nextY = wrapAngleSigned(rotationRef.current.y + vX * 0.8)
