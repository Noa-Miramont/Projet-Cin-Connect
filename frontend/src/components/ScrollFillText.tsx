...
export function ScrollFillText({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Start revealing when element enters at 85% of viewport
      // Complete when element reaches 25% from top
...
export function ScrollFillContainer({ children, className = '' }: ScrollFillContainerProps) {
  const [progress, setProgress] = useState<number>(0)
  const ref = useRef<HTMLDivElement | null>(null)
